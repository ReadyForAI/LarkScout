...
| 单元 | 行数 | 函数/类数 | 端点数 |
|---|---:|---:|---:|
| `services/docreader/larkscout_docreader.py` | **7894** | ~250 | 22 |
| `services/browser/larkscout_browser.py` | 2660 | ~70 | 13 |
| `providers/`（已分层，正面样板） | 533（4 文件） | — | — |
| `larkscout_server.py`（统一入口） | 75 | — | 1 |
...
### 非目标（已与维护者确认）
- **不拆成多进程/多服务**。继续 `larkscout_server.py` 单进程、`/web` + `/doc`、单端口 9898。模块边界按「未来可独立部署」来切，但当前不引入网络跳转、不引入独立部署编排。
...
| 耦合形式 | 量级 | 例子 |
|---|---:|---|
| `patch("larkscout_docreader.X")` / `monkeypatch.setattr(larkscout_docreader, ...)` | **~87 处** | `patch("larkscout_docreader._get_docs_dir")` ×50+ |
| `from larkscout_docreader import <内部符号>` | 数十处 | `Section`, `_split_sections`, `export_pdf_region_crop`, `_next_doc_id`, `_update_doc_index` … |
| `from larkscout_browser import <内部符号>` | 多处 | `_validate_url`, `Session`, `SessionManager`, `_next_web_doc_id` |
| 异类路径 | 1 处 | `services.docreader.larkscout_docreader.gemini_summarize`（test_sectioning_and_locale.py:51） |

### 3.1 `mock.patch` 的语义底线

`patch("larkscout_docreader._get_docs_dir")` 替换的是 **`larkscout_docreader` 模块对象上的属性**。它只能影响：
- 外部 `larkscout_docreader._get_docs_dir()` 形式的调用，**或**
- **`larkscout_docreader` 模块内部**以裸名 `_get_docs_dir()` 发起的调用（裸名在本模块全局命名空间查找）。

它**无法**影响一个子模块里以裸名调用自己 import 进来的同名函数（经典的 "patch where it's used, not where it's defined"）。

### 3.2 由此推导出的硬性设计规则

> **R1（入口即命名空间）**：`larkscout_docreader` / `larkscout_browser` 这两个**对外可被 import 与 patch 的顶层模块名必须保留**。拆分把它们从「单文件」变成「包」，但 `from larkscout_docreader import app/Section/...` 与 `patch("larkscout_docreader.X")` 必须照旧解析。

> **R2（被 patch 的符号留在调用现场）**：任何被测试 `patch`/`setattr` 的符号，其**调用方代码必须仍然执行在顶层模块（包 `__init__`）的命名空间内**，且该符号被 import 进顶层命名空间、以裸名调用。典型：`_get_docs_dir` 会迁到 `services/common`，但顶层模块 `from larkscout_common.storage import _get_docs_dir`，端点继续裸名调用 → `patch("larkscout_docreader._get_docs_dir")` 重绑顶层名后，端点看到的就是 mock。✅

> **R3（被直接 import 的符号一律 re-export）**：所有被 `from larkscout_docreader import X` 的纯函数/数据类（`Section`、`_split_sections`、`export_pdf_region_crop` …），在包 `__init__` 里 re-export，import 风格的测试零改动。

### 3.3 诚实的测试改动评估

不能承诺「零测试改动」。按 R1–R3，改动分三档：
- **安全（零改动）**：~50 处 `_get_docs_dir` patch（用于端点，符合 R2）+ 全部直接 import（符合 R3）。
- **需改 patch 目标路径（~15–20 处）**：被 patch 的符号若**只在子模块深处被调用**（如 `parse_pdf` 内部调用的 `gemini_ocr`、test_robustness/test_security/test_layout_sidecar_contract 里那批），patch 目标要从 `larkscout_docreader.X` 改成 `larkscout_docreader.<submodule>.X`。**与对应抽离 PR 同批修改**。
- **1 处异类路径**：`services.docreader.larkscout_docreader.gemini_summarize` 需统一到新结构路径。

> 每个抽离 PR 的 DoD 必须包含：「列出本 PR 移动的符号 → 检查其全部 patch/import 引用 → 同 PR 内更新失配的目标路径」。

## 4. 目标目录结构

保持现有 `sys.path` 挂载约定（`larkscout_server.py` 把两个 service 目录各自插入 `sys.path`；repo 根在 path 上，故 `i18n`、`providers`、新增 `larkscout_common` 均可裸名 import）。每个 monolith 文件 → **同名包**（满足 R1）。

```
larkscout/
├── larkscout_server.py                 # 不变：统一入口，mount /web /doc
├── i18n.py                             # 不变
├── providers/                         # 不变（已分层，样板）
│
├── larkscout_common/                  # ★ 新增：跨服务共享层（repo 根包，与 providers 平级）
│   ├── __init__.py
...
└── services/
    ├── browser/
    │   ├── larkscout_browser/         # ★ 单文件 → 包（保留可 import/patch 名 larkscout_browser）
    ...
    │
    └── docreader/
        ├── larkscout_docreader/       # ★ 单文件 → 包（保留可 import/patch 名 larkscout_docreader）
...
## 5. 共享层 `larkscout_common` 设计

最高收益/成本比，且是后续两包拆分的公共地基。**先做**。

- `storage.py`：迁入两边重复的 `_get_docs_dir`、`_normalize_content_type`、`_doc_storage_rel_path`、`_doc_storage_dir`，及常量 `CONTENT_TYPE_DIRS` / `_CONTENT_TYPE_ALIASES`。`DEFAULT_DOCS_DIR` 统一为单一定义（读 `LARKSCOUT_DOCS_DIR`，默认 `~/.larkscout/docs`），两边现有的 `DEFAULT_DOCS_DIR` / `_DEFAULT_DOCS_DIR` 改为引用它。
...
**命名空间兼容**：browser/docreader 顶层 `__init__` 通过 `from larkscout_common.storage import _get_docs_dir, _normalize_content_type, ...` 把这些名 import 进各自命名空间（满足 R2，`patch("larkscout_docreader._get_docs_dir")` 与 `patch("larkscout_browser._get_docs_dir")` 继续生效）。
...
| **1** | 抽 `larkscout_common`（storage/atomic/paths）；browser+docreader 顶层改为 import 共享层，删除两边重复定义 | 全测试绿；`_get_docs_dir` patch 仍命中 | 中：DEFAULT_DOCS_DIR 统一、锁不迁移 |
| **2** | docreader：`larkscout_docreader.py` → 包；先迁**叶子** `models.py` + `text_utils.py`（零跨模块回调） | `from larkscout_docreader import Section/PageContent/...` 绿 | 低 |
...
| **6** | browser：`larkscout_browser.py` → 包；迁 `models.py`+`security.py`+`session.py`（叶子） | test_security `_validate_url` / test_concurrency `Session` 绿 | 低 |
| **7** | browser：抽 `vision.py`+`routing.py`+`actions.py`+`webmcp.py`+`distill.py`+`capture.py`；conftest `patch("larkscout_browser.async_playwright")` 目标核对 | test_web_capture（`_distill`/`_browser` patch）绿 | 中：`_browser` 全局、playwright 生命周期 |
...
- **命名空间回归**：保留一个轻量 smoke 测试断言 `from larkscout_docreader import app, Section, _split_sections, _get_docs_dir` 与 `from larkscout_browser import app, _validate_url` 全部可解析——防止门面 re-export 漏项。
- **行为零变更证据**：PR 不新增/修改断言逻辑，仅在「被 patch 符号迁出顶层模块」时更新 patch 目标路径，且在 PR 描述列明每一处路径变更。
- **打包**：PR 8 后 `pip install -e . && python -c "import larkscout_server"` 通过。

## 10. 待 review 的决策点

1. 共享层命名：`larkscout_common`（与 `providers` 平级的 repo 根包）vs 放 `services/common`（需把 `services/` 加入 `sys.path`）。本文选前者，因与现有 `i18n.py`/`providers/` 的 repo 根约定一致，**无需改 sys.path**。
